...
public record ThreadExceptionHandel (long ThreadID, String ThreadName, long CountTime) {
    ...
    /**
     * Retrieves a list of strings representing the count time.
     *
     * @param Time The time taken for the thread to execute.
     * @return A list of strings, where each string is a line containing the count time.
     */
    public List<String> getResult(long Time) {
        return String.valueOf(CountTime).lines().toList();
    }
}
